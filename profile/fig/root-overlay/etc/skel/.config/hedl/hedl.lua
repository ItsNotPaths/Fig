-- fig on hedl. This replaces the sway config it grew out of.
--
...
-- The clipboard preview, and only it. mpv is told to wear a different app id
-- for this one job, so an mpv opened to watch something is an ordinary tiled
-- window and this one is not.
--
-- Wayland gives a client no way to place itself, and a window rule carries no
-- geometry, so the position is set on map. Under the picker, which is 15 rows
-- at the top of the screen.
hedl.window_rule({ class = "^fig-clip-preview$" }, { floating = true, borderpx = 0 })

-- `appid` and not `class`: a rule matches on `class`, a client answers to
-- `appid`, and the two names are for the same string.
hedl.on("map", function(c)
  if c.appid ~= "fig-clip-preview" then return end
  local m = hedl.monitor()
  if not m then return end
  local w = math.floor(m.width * 0.5)
  local h = math.floor(m.height * 0.4)
  -- One call, so the window is never briefly the wrong size on the way to
  -- being the right one.
  c:place(m.x + math.floor((m.width - w) / 2),
          m.y + math.floor(m.height * 0.42), w, h)
end)

hedl.bind(mod .. " + Return",       "Terminal",     hedl.dsp.spawn(term))
hedl.bind(mod .. " + Q",            "Close window", hedl.dsp.killclient())
hedl.bind(mod .. " + D",            "Run",          hedl.dsp.spawn("wweft ~/.config/figshell/apps.lua"))
...
hedl.bind(mod .. " + A",            "Menu",         hedl.dsp.spawn("wweft ~/.config/figshell/menu.lua"))
-- What every key does, off the binds hedl publishes. This one included.
hedl.bind(mod .. " + K",            "Keys",         hedl.dsp.spawn("wweft ~/.config/figshell/keys.lua"))
hedl.bind(mod .. " + SHIFT + T",    "Theme",        hedl.dsp.spawn("wweft ~/.config/figshell/theme-picker.lua"))
-- C and not V: V is "Float all", further down, and a second bind on one key
-- replaces the first rather than complaining about it.
hedl.bind(mod .. " + C",            "Clipboard",    hedl.dsp.spawn("wweft ~/.config/figshell/clipboard.lua"))
...
hedl.bind(mod .. " + SHIFT + R",    "Reload", function()
  hedl.dsp.reload()()
  hedl.dsp.spawn("pkill -x wweft; wweft ~/.config/figshell/bar.lua")()
end)
-- The screen lock. The same verb the menu row and the idle timeout call, so
-- the timeout and the key can never end up locking with different things.
hedl.bind(mod .. " + SHIFT + L",    "Lock",         hedl.dsp.spawn("bar-actions lock"))
